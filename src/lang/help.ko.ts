...
export const helpKo = {
        ...
        "risuBardAutoConfirmLagTurns": "자동 위키 확정을 이만큼 더 과거의 응답으로 물립니다. 0 이면 직전 응답을 확정합니다(기본). 최근 턴은 원문이 이미 컨텍스트에 들어가므로 위키와 중복이고, 확정된 턴은 리롤할 때 위키를 되돌려야 합니다. 값을 올리면 그만큼 리롤이 자유로워지는 대신, 대화를 멈춘 시점의 마지막 구간은 수동 갱신으로 메꿔야 합니다.",
